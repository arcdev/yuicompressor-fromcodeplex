...
namespace Yahoo.Yui.Compressor.MsBuildTask
{
    public abstract class CompressorTask : Task
    {
        ...
        protected CompressorTask(ICompressor compressor) : this()
        {
        ...
        public override bool Execute()
        {
            ...
            if (loggingType == Yui.Compressor.LoggingType.Debug)
            {
                LogTaskParameters();
            }
            ...
            StringBuilder compressedText = this.CompressFiles();
        ...
        protected virtual void ParseBuildParameters()
        {
            ParseLoggingType();
        ...
        {
            if (loggingType != Yui.Compressor.LoggingType.None)
            {
        ...
        private void ParseLoggingType()
        {
            ...
            {
                this.loggingType = Yui.Compressor.LoggingType.Info;
            ...
            {
                case "none":
                    this.loggingType = Yui.Compressor.LoggingType.None;
                    break;
                case "debug":
                    this.loggingType = Yui.Compressor.LoggingType.Debug;
                    break;
                case "info":
                    this.loggingType = Yui.Compressor.LoggingType.Info;
                    break;
        ...
        private StringBuilder CompressFiles()
        {
            ...
            if (SourceFiles != null)
            {
                ...
                {
                    var message = "=> " + file.ItemSpec.Trim();

                    // Load up the file.
                    try
                    {
                        var originalContent = File.ReadAllText(file.ItemSpec.Trim());
                        totalOriginalContentLength += originalContent.Length;

                        if (string.IsNullOrEmpty(originalContent))
                        {
                            LogMessage(message, true);
                            Log.LogError(string.Format(CultureInfo.InvariantCulture, "There is no data in the file [{0}]. Please check that this is the file you want to compress.", file));
                        }
                    ...
                    catch (Exception exception)
                    {
                        ...
                        {
                            Log.LogError(string.Format(CultureInfo.InvariantCulture, "ERROR reading file or path [{0}].", file));
                        }
                        else
                        {
                            // FFS :( Something bad happened.
                            Log.LogError(string.Format(CultureInfo.InvariantCulture, "Failed to read/parse data in file [{0}].", file));
                        }
        ...
        protected virtual string Compress(ITaskItem file, string originalContent)
        {
        ...
        private CompressionType GetCompressionTypeFor(ITaskItem file)
        {
            var message = "=> " + file.ItemSpec;
            var actualCompressionType = this.compressionType;
            var overrideType = file.GetMetadata("CompressionType");
