...
namespace Yahoo.Yui.Compressor
{
    public class MichaelAshRegexCompressor
    {
        ...
        public static string Compress(string css, int columnWidth, bool removeComments)
        {
            ...
            */
            css = Regex.Replace(css, @"(?#no preceding space needed)\s+((?:[!{};>+()\],])|(?<={[^{}]*):(?=[^}]*}))",
                                "$1");
            ...
            css = Regex.Replace(css, @"(\d+)\.0+(p(?:[xct])|(?:[cem])m|%|in|ex)\b", "$1$2");
                // Remove .0 from size units x.0em becomes xem
            ...
            css = Regex.Replace(css,
                                @":\s*(inherit|auto|0|(?:(?:\d*\.?\d+(?:p(?:[xct])|(?:[cem])m|%|in|ex))))(\s+\1){1,3};",
                                ":$1;", RegexOptions.IgnoreCase);
            // if has 4 parameters and top unit = bottom unit and right unit = left unit make 2 parameters
            css = Regex.Replace(css,
                                @":\s*((inherit|auto|0|(?:(?:\d*\.?\d+(?:p(?:[xct])|(?:[cem])m|%|in|ex))))\s+(inherit|auto|0|(?:(?:\d?\.?\d(?:p(?:[xct])|(?:[cem])m|%|in|ex)))))\s+\2\s+\3;",
                                ":$1;", RegexOptions.IgnoreCase);
            // if has 4 parameters and top unit != bottom unit and right unit = left unit make 3 parameters
            css = Regex.Replace(css,
                                @":\s*((?:(?:inherit|auto|0|(?:(?:\d*\.?\d+(?:p(?:[xct])|(?:[cem])m|%|in|ex))))\s+)?(inherit|auto|0|(?:(?:\d?\.?\d(?:p(?:[xct])|(?:[cem])m|%|in|ex))))\s+(?:0|(?:(?:\d?\.?\d(?:p(?:[xct])|(?:[cem])m|%|in|ex)))))\s+\2;",
                                ":$1;", RegexOptions.IgnoreCase);
            ...
            css = Regex.Replace(css,
                                ...
                                "$1-style:$2;", RegexOptions.IgnoreCase);
            ...
            css = Regex.Replace(css,
                                @"(outline|border)-style\s*:\s*((none|hidden|d(?:otted|ashed|ouble)|solid|groove|ridge|inset|outset)\s+(?:none|hidden|d(?:otted|ashed|ouble)|solid|groove|ridge|inset|outset ))(?:\s+\3);",
                                "$1-style:$2;", RegexOptions.IgnoreCase);

            //  Outline-color and Border-color parameter reduction
            css = Regex.Replace(css, @"(outline|border)-color\s*:\s*((?:\#(?:[0-9A-F]{3}){1,2})|\S+)(?:\s+\2){1,3};",
                                "$1-color:$2;", RegexOptions.IgnoreCase);
            ...
            css = Regex.Replace(css,
                                ...
                                rgbDelegate);
            css = Regex.Replace(css, @"(?<![\x22\x27=]\s*)\#(?:([0-9A-F])\1)(?:([0-9A-F])\2)(?:([0-9A-F])\3)", "#$1$2$3",
            ...
            css = Regex.Replace(css, @"(?<=color\s*:\s*.*)\#(?<hex>[0-9a-f]{6})", shortColorNameDelegate,
                                RegexOptions.IgnoreCase);
        ...
        private static string ShortColorNameMatchHandler(Match m)
        {
            ...
            if (_shortColorNames.ContainsKey(m.Groups["hex"].Value))
            {
                returnValue = _shortColorNames[m.Groups["hex"].Value].ToString();
            }
            return returnValue;
        }
        ...
        private static string ShortColorHexMatchHandler(Match m)
        {
            ...
            return _shortHexColors[m.Value.ToLower()].ToString();
        }
            ...
            if (_shortColorNames == null)
            {
                lock (SyncObject)
                {
                    if (_shortColorNames == null)
                    {
                        ...
                        _shortColorNames = shortColorNames;
                    }
                }
            }

            if (_shortHexColors == null)
            {
                lock (SyncObject)
                {
                    if (_shortHexColors == null)
                    {
                        ...
                        _shortHexColors = shortHexColors;
                    }
                }
            }
        }
        ...
        private static string ShortHandAllProperties(string css)
        {
             ...
             */
            var reCSSBlock = new Regex("{[^{}]*}");
            var reTRBL1 =
                new Regex(
                    @"(?<fullProperty>(?:(?<property>padding)-(?<position>top|right|bottom|left)))\s*:\s*(?<unit>[\w.]+);?",
                    RegexOptions.IgnoreCase);
            var reTRBL2 =
                new Regex(
                    @"(?<fullProperty>(?:(?<property>margin)-(?<position>top|right|bottom|left)))\s*:\s*(?<unit>[\w.]+);?",
                    RegexOptions.IgnoreCase);
            var reTRBL3 =
                new Regex(
                    @"(?<fullProperty>(?<property>border)-(?<position>top|right|bottom|left)(?<property2>-(?:color)))\s*:\s*(?<unit>[#\w.]+);?",
                    RegexOptions.IgnoreCase);
            var reTRBL4 =
                new Regex(
                    @"(?<fullProperty>(?<property>border)-(?<position>top|right|bottom|left)(?<property2>-(?:style)))\s*:\s*(?<unit>none|hidden|d(?:otted|ashed|ouble)|solid|groove|ridge|inset|outset);?",
                    RegexOptions.IgnoreCase);
            var reTRBL5 =
                new Regex(
                    @"(?<fullProperty>(?<property>border)-(?<position>top|right|bottom|left)(?<property2>-(?:width)))\s*:\s*(?<unit>[\w.]+);?",
                    RegexOptions.IgnoreCase);
            var reListStyle = new Regex(@"list-style-(?<style>type|image|position)\s*:\s*(?<unit>[^};]+);?",
                                        RegexOptions.IgnoreCase);
            var reFont =
                new Regex(
                    @"font-(?:(?:(?<fontProperty>family\b)\s*:\s*(?<fontPropertyValue>(?:\b[a-zA-Z]+(-[a-zA-Z]+)?\b|\x22[^\x22]+\x22)(?:\s*,\s*(?:\b[a-zA-Z]+(-[a-zA-Z]+)?\b|\x22[^\x22]+\x22))*)\b)|(?:(?<fontProperty>style\b)\s*:\s*(?<fontPropertyValue>normal|italic|oblique|inherit))|(?:(?<fontProperty>variant\b)\s*:\s*(?<fontPropertyValue>normal|small-caps|inherit))|(?:(?<fontProperty>weight\b)\s*:\s*(?<fontPropertyValue>normal|bold|(?:bold|light)er|[1-9]00|inherit))|(?:(?<fontProperty>size\b)\s*:\s*(?<fontPropertyValue>(?:(?:xx?-)?(?:small|large))|medium|(?:\d*\.?\d+(?:%|(p(?:[xct])|(?:[cem])m|in|ex))\b)|inherit|\b0\b)))\s*;?",
                    (RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace));
            var reBackGround =
                new Regex(
                    @"background-(?:(?:(?<property>color)\s*:\s*(?<unit>transparent|inherit|(?:(?:\#(?:[0-9A-F]{3}){1,2})|\S+)))|(?:(?<property>image)\s*:\s*(?<unit>none|inherit|(?:url\s*\([^()]+\))))|(?:(?<property>repeat)\s*:\s*(?<unit>no-repeat|inherit|repeat(?:-[xy])))|(?:(?<property>attachment)\s*:\s*(?<unit>scroll|inherit|fixed))|(?:(?<property>position)\s*:\s*(?<unit>((?<horizontal>left | center | right|(?:0|(?:(?:\d*\.?\d+(?:p(?:[xct])|(?:[cem])m|%|in|ex)))))\s+(?<vertical>top | center | bottom |(?:0|(?:(?:\d*\.?\d+(?:p(?:[xct])|(?:[cem])m|%|in|ex))))))|((?<vertical>top | center | bottom )\s+(?<horizontal>left | center | right ))|((?<horizontal>left | center | right )|(?<vertical>top | center | bottom )))));?",
        ...
        private static string ShortHandFontReplaceV2(MatchCollection mcProperySet, Regex re, string InputText)
        {
            ...
            var reLineHeight =
                new Regex(@"line-height\s*:\s*((?:\d*\.?\d+(?:%|(p(?:[xct])|(?:[cem])m|in|ex)\b)?)|normal|inherit);?",
                          RegexOptions.IgnoreCase);
            ...
            string strShortcut;
            string strProperties = string.Format("{0}{1}{2};", strStyle_Variant_Weight, strVariant, strWeight, strSize,
                                                 strFamily);
        ...
        private static string ShortHandBackGroundReplaceV2(MatchCollection mcProperySet, Regex re, string InputText)
        {
            ...
            string strShortcut;
            string strProperties = string.Format("{0}{1}{2}{3}{4};", strColor, strImage, strRepeat, strAttachment,
                                                 strPosition);
        ...
        private static string ShortHandReplaceV2(MatchCollection mcProperySet, Regex reTRBL1, string InputText)
        {
            ...
            string strProperty;
            strProperty = string.Format("{0}{1}", mcProperySet[0].Groups["property"].Value,
                                        mcProperySet[0].Groups["property2"].Value);
            foreach (Match mProperty in mcProperySet)
            {
